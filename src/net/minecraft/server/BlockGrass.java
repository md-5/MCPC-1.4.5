...
public class BlockGrass extends Block {
    ...
    public void b(World world, int i, int j, int k, Random random) {
        if (!world.isStatic) {
            if (world.getLightLevel(i, j + 1, k) < 4 && world.b(i, j + 1, k) > 2) { // Forge
                // CraftBukkit start
                ...
                for (int l = 0; l < Math.max(4, Math.max(20, (int) (4 * 100F / world.growthOdds))); ++l) { // Spigot
                    ...
                    if (world.getTypeId(i1, j1, k1) == Block.DIRT.id && world.getLightLevel(i1, j1 + 1, k1) >= 4 && world.b(i1, j1 + 1, k1) <= 2) { // Forge
                        // CraftBukkit start
                        org.bukkit.World bworld = world.getWorld();
